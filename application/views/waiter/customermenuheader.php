...
function Notification()
{
  ...
  $.ajax({
    url:"<?php echo site_url('waiter/orders/notification'); ?>",
    type:"POST",
...
function blink()
{
  $("#red").css("background-color","#FFA500");
  setTimeout("blink2()",1000);
}
function blink2()
{
  $("#red").css("background-color","#e24813");
...
{
  $.ajax({
    url:"<?php echo site_url('waiter/orders/getHelp'); ?>",
    type:"POST",
    success:function(data){
      $("#mainVideoMsg").slideDown();
      //$("#mainVideoMsg").show();
      $("#content").html(data)
...
<div class="tablenumber_kitchen_screen_fix rounded-4px pull-left"
 ...
 padding: 29px 10px 10px 10px !important; margin-top: 24px !important;">
  ...
  <div class="titlebar">
    <span class="title">Restaurant Saffron</span>
      ...
      <div class="">
        ...
        <li class="link_button1" >
           <div >
            ...
            <i class="icon-user icon-white"></i> Name <?php echo $this->session->userdata('customername'); ?>
...
<script type="text/javascript">
  ...
  $('.modal-popup').setupModal({id: 'popMod1'});
  $('.modal-popup-type2').setupModal({id: 'popMod2', modal: false, transition:'slideDown'});
  //$('.modal-popup').setupModal({modal:false, transition: function() {alert('show me');}});
