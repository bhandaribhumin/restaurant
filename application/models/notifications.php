...
var log_id=<?php echo $this->data['id'];?>;
var j_array = new Array();
    <?php foreach($id_array as $key => $val){ ?>
        j_array.push('<?php echo $val; ?>');
    <?php } ?>
    
jQuery.each(j_array, function(index, value) {
       
       if(value==log_id){

            var id=log_id;
            alert(id);
           $('#as'+id).css('background-color','red');
           // $('#as'+987).css('background-color','red');
       }
