...
class orderitem_model extends CI_Model{
    ...
    public function add($order_id,$menu_id,$tablenumber=null,$qty=null,$price,$notes=null,$sl,$el){
   ...
   public function get_total_with_tax($order_id){
        ...
        $query=$this->db->query("SELECT
            (SUM(orderitem.`price`)*$tax)/100 AS total FROM orderitem 
     ...
     public function get_total_without_tax($order_id){
        ...
        // exit();
        $query=$this->db->query("SELECT
            (SUM(orderitem.`price`)) AS total FROM orderitem 
                    JOIN `order` ON `order`.id = orderitem.orderid
                    WHERE
                     `order`.payment_status=0 and
                        `order`.id ='".$order_id."'");
