...
class Dashboard extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data['menuitems']=$this->menuitem->get_all_available_menu_item();
 ...
 $drop="<label for='exampleInputName2'>Table No</label>
	...
	public function placeOrder(){
		...
		if(isset($_POST['order_id'])!=''){
			...
			$order_id=$this->order->add_order($tablenumber,$waiter_id,$customer_name,$mobile_no);
		}
		...
		for($i=1;$i<=$total_item;$i++){
				...
				if(isset($_POST['sl_'.$i]))
					...
					if($order_id != ''){
						...
						$this->orderitem->add($order_id,$menu_id,$tablenumber,$qty,$price,$notes,$sl,$el);
					}
