...
class Menu extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->data['currencys']  = $this->usermodel->getcurrency();
		$this->currencys = $this->usermodel->getcurrency();
		$this->currencys = $this->currencys->currency;
	}
	...
	public function index()
	{
		...
		$this->data['menu_beverages'] = $this->menuitems->get_menuitem_beverages_type();
		$this->data['menu_specialoffer'] = $this->menuitems->get_menuitem_specialoffer_type();
		$this->data['menu_shisha'] = $this->menuitems->get_menuitem_shisha_type();
		//$this->load->model('news_model');
		//$this->data['news'] = $this->news_model->get_news();
		if(!$this->session->userdata('tablenumber') && !$this->session->userdata('assign_waiter'))
		{
			$this->data['yourorder_total'] = "";
		}else
		{
			$tablenumber = $this->session->userdata('tablenumber');
			$waiter_id = $this->session->userdata('assign_waiter');
			$q = $this->db->query("select * from temp_your_order where table_no = '".$tablenumber."' and waiter_id = '".$waiter_id."'")->result();
			//echo $this->db->last_query();
			//print_r($q);
			//exit();
			$this->data['yourorder_total'] = $q;
	...
	public function logout()
	{
		...
		$tablenumber = $this->session->userdata('tablenumber');
		$query = $this->db->query("select * from `table` where tablenumber = $tablenumber AND inuse = 1")->row();
		
		if(!empty($query))
		{
			$data = array('inuse' => 0);
			$condition = array('tablenumber' => $tablenumber);
			$this->db->update('table', $data , $condition);
		}
		

		$this->session->unset_userdata('tablenumber');
		$this->session->unset_userdata('tabletnumber');
		$this->session->unset_userdata('assign_waiter');
		$this->session->unset_userdata('inuse');
	...
	public function notification(){
      ...
      /*	echo "<pre>";
		...
		$tempitems_html .= "<div class=ordereditem><div class='ordered pull-left'>";
			echo "<b>$this->currencys $unpaid_item->price</b>";						
		...
		echo "<hr style='width:200px;'>$this->currencys; $price_items_sum->price &nbsp;&nbsp; Total";
			echo $tempitems_html;
	...
	foreach ($tempitems as $tempitem){
		...
		$temp_data_q=$quantitys->row();	
			  ...
			  $tempitems_html .= "<div notes='$tempitem->notes' quantity='$tempitem->quantity' ingredients='$tempitem->ingredients' price='$tempitem->price' itemid='$tempitem->itemid' class='ordereditem'>";
           $tempitems_html .= "<b> $tempitem->itemname</b>";
            $tempitems_html .= "<img src=$tempitem->item_img width='100' height='100' class='img-polaroid pull-left menuimg'><br/>";
            if($tempitem->quantity != 1)
       			{

            $tempitems_html .= "<b>Price : $this->currencys $temp_data->price</b><br>";
            $tempitems_html .= "<b>Total: $this->currencys ";
       		 }else
       		 {
       		 	$tempitems_html .= "<b>Price: $this->currencys";
       		 }

            $tempitems_html .= " $tempitem->price </b>";
            if($tempitem->notes != '' && $tempitem->notes != 0)
            {
             $tempitems_html .= "<br>Comment: <b> <input style='color:red; width:auto;' id=demo_notes value=$tempitem->notes type=text name=demo_notes readonly></b>        
            
			";
		}
            $tempitems_html .= "<br>Quantity: <b> <input style='color:red; width:20px;' id=demo3 value=$tempitem->quantity type=text name=demo_vertical2 readonly></b>        
			<br/>
	...
	/*$tempitems_html .= "<div style='background: none repeat scroll 0 0 #f3f3f3;font-size: 31px;
height: auto;
padding: 10px;
    font-weight: 700;'>
			<span >Total :   </span>
			<span class='pull-right' id=ordertotal>$this->currencys $total .00</span></div>";
*/
	echo $tempitems_html;
        }
	...
	foreach ($tempitems as $tempitem){
		...
		$temp_data_q=$quantitys->row();	
			  ...
			  $tempitems_html .= "<div notes='$tempitem->notes' quantity='$tempitem->quantity' ingredients='$tempitem->ingredients' price='$tempitem->price' itemid='$tempitem->itemid' class='ordereditem'>";
           //$tempitems_html .= "<b> $tempitem->itemname</b>";
           $tempitems_html .= "<img src=$tempitem->item_img width='100' height='100' class='img-polaroid pull-left menuimg'><span class='ordered'>$tempitem->itemname <br/>";
            if($tempitem->quantity != 1)
       			{
            $tempitems_html .= "<b>Price : $this->currencys $temp_data->price</b><br>";
       		$tempitems_html .= "<b>Total: $this->currencys ";
		...
		$temp_data=$quantitys->row();	
          ...
          // $tempitems_html .= "<b> $tempitem->itemname</b>";
           $tempitems_html .= "<img src=$tempitem->item_img width='100' height='100' class='img-polaroid pull-left menuimg'><span class='ordered'>$tempitem->itemname <br/>";
            if($tempitem->quantity != 1)
       			{
            $tempitems_html .= "<b>Price : $this->currencys $menuid->price</b><br>";
       		 $tempitems_html .= "<b>Total: $this->currencys";
       		 }else
       		 {
       		 	$tempitems_html .= "<b>Price: $this->currencys";
            ...
            $tempitems_html .= "<br>Quantity: <b> <input style='color:red; width:20px;' id=demo3 value=$tempitem->quantity type=text name=demo_vertical2 readonly>
	...
	foreach ($tempitems as $tempitem){
		...
		//print_r($this->db->last_query());
           ...
           // $tempitems_html .= "<b> $tempitem->itemname</b>";
            ...
            $tempitems_html .= "<br>Quantity: <b>
             <input style='color:red; width:20px;' id=demo3 value=$tempitem->quantity type=text name=demo_vertical2 readonly>
             <button price='$menuid->price' quantity='$tempitem->quantity' itemid='$tempitem->itemid' onclick='quantityAdd(event)' class=bootstrap-touchspin-down type=button>+</button>
              ";
...
*/	echo $tempitems_html;
	...
	/*$quantitys = $this->db->query("select * from temp_your_order where itemid=$menuitem->id");
		...
		$temp_data=$quantitys->row();
			...
			if ($index % 2 == 0){
				$menuitems_html .= "<div class='menu-item span6 menuitem' style='margin-left:0'>";
				...
				$menuitems_html .= "<span class='' ><b>" . $menuitem->name ." Desc :". $menuitem->description ."</b></span><br><span>&nbsp;<b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/><a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
				...
				$menuitems_html .= "
				<a style='float:right;height:34px;margin:-55px 16px; font-size:24px;' href='#' itemid='" . $menuitem->id . "' ingredients='All' itemimg='" . $img_path . $menuitem->picturepath . "' onclick='addToOrder(event)' class='btn btn-primary btn-plus' itemname='" . $menuitem->name . "' price='" . $menuitem->price . "'><i class='icon-plus'></i> Add to Order</a>
				&nbsp;&nbsp;<br></div>";
				...
				$menuitems_html .= "<div class='menu-item span6 menuitem'>";
				if ($menuitem->picturepath == '' || $menuitem->picturepath == null){
					$menuitems_html .= "<img src='" . base_url() . "assets/img/140x140.gif" . "' class='menuimg pull-left img-polaroid' alt='" . $menuitem->name . "' />";
				} else {
					$menuitems_html .= "<img src='" . $img_path . $menuitem->picturepath . "' class='menuimg pull-left img-polaroid' alt='" . $menuitem->name . "' />";
				}		
				$menuitems_html .= "<span>" . $menuitem->name ." Desc :". $menuitem->description ."</span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/>
	...
	/*$quantitys = $this->db->query("select * from temp_your_order where itemid=$menuitem->id");
		...
		$temp_data=$quantitys->row();
			...
			if ($index % 2 == 0){
				...
				$menuitems_html .= "<span><b>" . $menuitem->name ." </b><br><b><i style='color:#fd5c2a;'>". $menuitem->description ."</i></b></span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br><br/><br/><a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
				...
				$menuitems_html .= "
				<a style='float:right;height:34px;margin:-55px 16px; font-size:24px;' href='#' itemid='" . $menuitem->id . "' ingredients='All' itemimg='" . $img_path . $menuitem->picturepath . "' onclick='addToOrder(event)'
				 class='btn btn-primary btn-plus btn-cart-img' itemname='" . $menuitem->name . "' price='" . $menuitem->price . "'>
				<i class='icon-plus'></i> Add to Order</a>
				&nbsp;&nbsp;<br><br></div>";
				//<a href='#customize-modal' itemimg='" . $img_path . $menuitem->picturepath . "' onclick='getIngredients(event)' itemprice='" . $menuitem->price . "' itemname='" . $menuitem->name . "' itemid='" . $menuitem->id . "' class='btn btn-custom btn-customize' data-toggle='modal'><i class='icon-wrench'></i> Customize</a>
				//</div>";
				...
				$menuitems_html .= "<span><b>" . $menuitem->name ." </b><br><b><i style='color:#fd5c2a;'>". $menuitem->description ."</i></b></span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/><br>
				<a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
...
public function get_beverages_menu_items()
	{
		if (!$this->input->is_ajax_request()){
			redirect('customer/menu', 'refresh');
		}
		...
		$type = $this->input->post('type');
		$menuitems = $this->menuitems->get_beverages_menuitem_by_type($type);
		$menuitems_html = '';
		$img_path = $this->config->item('img_path');
		$index = 0;
	...
	/*$quantitys = $this->db->query("select * from temp_your_order where itemid=$menuitem->id");
		...
		$temp_data=$quantitys->row();
			...
			if ($index % 2 == 0){
				...
				$menuitems_html .= "<span>" . $menuitem->name ." <br><b><i style='color:#fd5c2a;'>". $menuitem->description ."</i></b></span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/><a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
				...
				$menuitems_html .= "<span>" . $menuitem->name ." <br><b><i style='color:#fd5c2a;'>". $menuitem->description ."</i></b></span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/>
				<a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
	...
	public function getSpecialOfferMenuItems()
	{
		if (!$this->input->is_ajax_request()){
			redirect('customer/menu', 'refresh');
		}
		...
		$type = $this->input->post('type');
		$menuitems = $this->menuitems->get_offer_menuitem_by_type($type);
		$menuitems_html = '';
		$img_path = $this->config->item('img_path');
		$index = 0;
...
public function getShishaMenuItems()
	{
		if (!$this->input->is_ajax_request()){
			redirect('customer/menu', 'refresh');
		}
		...
		if (empty($menuitems)){
			...
			foreach ($menuitems as $menuitem){
				if ($index % 2 == 0){
					...
					<span>" . $menuitem->name ." <br><b><i style='color:#fd5c2a;'>". $menuitem->description ."</i></b></span>&nbsp;&nbsp;<br/><span><b class='menu-price'>$this->currencys " . $menuitem->price . "</b></span><br/><br/>
					<a href='#moreinfo-modal' itemid='" . $menuitem->id . "' onclick='getMoreInfo(event)' class='btn btn-moreinfo btn-moreinfo' data-toggle='modal'>"; 
	...
	public function placeorder()
	{
		...
		$this->session->set_flashdata('successmsg', "Your ORder Profile successfully.");

		$this->db->query("DELETE FROM temp_your_order where table_no= '".$this->session->userdata('tablenumber')."' AND waiter_id = '".$this->session->userdata('assign_waiter')."'");
	...
	public function getingredients()
	{
		...
		if (empty($ingredients)){
			...
			$ingredients_html = "<p class='contentbox rounded-4px'>Please indicate the items you do not want by deselecting the appropriate checkeboxes.
							...
							$ingredients_html = "<input type='text' id='comment_ingredients' name='comment_ingredients'>";
			foreach ($ingredients as $ingredient){
				$ing_sp = explode(",", $ingredient->keywords);
				foreach ($ing_sp as $ingredient_list){

				$ingredients_html .= "<div class='ingredientbox'><input class='ingr_box' onclick='selectIngredient(event)' ingrname='" . $ingredient_list . "' type='checkbox' checked/> " . $ingredient_list . "</div>";
			}
		}
		}
